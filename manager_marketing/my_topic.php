...
//$topic = $conn->query("SELECT faculty.*,user.*, topic.* FROM (( faculty INNER JOIN topic ON faculty.f_id = topic.faculty_id) INNER JOIN user ON faculty.f_id = user.faculty_id) WHERE user.u_id = '$userId' AND user.role = 'manager-marketing'");
printf("SELECT faculty.*,user.*, topic.* FROM (( faculty INNER JOIN topic ON faculty.f_id = topic.faculty_id) INNER JOIN user ON faculty.f_id = user.faculty_id) WHERE user.u_id = '$userId' AND user.role = 'manager-marketing'");
                            ...
                            <div class="card-body">
                                ...
                                <div class="table-responsive p-t-10">
                                    <table class="table text-center">
                                        ...
                                        $topicInfor = $conn->query("SELECT user.*, topic.*, faculty.* FROM topic INNER JOIN user ON user.u_id = user.u_id INNER JOIN faculty ON faculty.f_id = user.faculty_id WHERE user.u_id = '$userId' AND user.role = 'manager-marketing'");
                                        ...
                                        foreach ($topicFacultyInfor as $row) {
                                            ...
                                            <tr>
                                                ...
                                                ?>
                                                <td><a type="button" class="btn btn-primary" href="manage_article.php?idt=<?= $row["id"] ?>">Select</a>
                                                </td>
                                            </tr>
                                            <?php
                                        }
